                                ...
                                <div class="container-fluid" style="margin-bottom: 25px;">
                                    <div class="row">
                                        <div class="offset-2 col-sm-8 bg-info p-2">
                                          <form action="{{route('Backendguardian-recommendation.update',$data->id)}}" method="post" enctype="multipart/form-data">
                                            <h2>Add Guardian recommendation</h2>
                                            @csrf
                                            @method('patch')
                                                <div class="form-group">
                                                    <label for="background">Name</label>
                                                    <input type="text" class="form-control" id="background" name="name"  placeholder="Enter Name" value="{{$data->name}}">
                                                </div>
                                                <div class="form-group">
                                                    <label for="background">Image</label>
                                                    <img src="{{$data->image}}" alt="" style="80px">
                                                    <input type="file" class="form-control" id="background" name="image"  placeholder="Enter background" value="{{ old('background')}}">
                                                </div>
                                                <div class="form-group">
                                                    <label for="subject">Description</label>
                                                    <textarea name="description" class="form-control"  placeholder="Enter Recommandation">{!! $data->description !!}</textarea>
                                                </div>
                                                <div class="form-group">
